...
export class LocalProvider implements Provider {
  ...
  async #getPages(lang: string, version: string): Promise<PageIndexInfo[]> {
    ...
    const segments = [
      `${SEARCH_INDEX_NAME}${searchIndexVersion}${searchIndexLang}`,
      `${this.#searchIndexHash[searchIndexGroupID]}`,
      'json',
    ]

    const filename = segments.filter(Boolean).join('.')

    const searchIndexURL = `/search-index/${filename}`
  ...
  async init(options: SearchOptions) {
    const pagesForSearch: PageIndexForFlexSearch[] = (await this.fetchSearchIndex(options)).map(
      (page) => ({
        ...page,
