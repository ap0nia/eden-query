...
import { LOCAL_INDEX, type NormalizedSearchResultItem, type Provider } from './Provider'
...
import { backTrackHeaders, byteToCharIndex, getStrByteLength, normalizeTextCase } from './util'
...
export class PageSearcher {
  ...
  #matchResultItem(normalizedKeyWord: string, resultItem: NormalizedSearchResultItem) {
  ...
  #matchHeader(item: PageIndexInfo, query: string, matchedResult: DefaultMatchResultItem[]) {
    /**
  ...
  #matchContent(
    ...
    const isHeaderMatched = (header: Header) => header && matchHeaderSet?.has(header)
    ...
    if (queryIndex === -1) {
      ...
      const highlightStartIndex = (item as RemotePageInfo)._matchesPosition.content[0]!.start
      ...
      const highlightInfoList = (item as RemotePageInfo)._matchesPosition.content
        .filter(
          (match) =>
            match.start >= highlightStartIndex &&
            match.start + match.length <= highlightStartIndex + THRESHOLD_CONTENT_LENGTH,
        )
        .map((match) => {
