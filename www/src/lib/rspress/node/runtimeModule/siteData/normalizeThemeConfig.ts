...
export function normalizeThemeConfig(docConfig: UserConfig): NormalizedDefaultThemeConfig {
    ...
    const normalizeSidebarItem = (
      ...
      if (typeof item === 'object' && 'items' in item) {
        return {
          ...
          ...('link' in item && item.link ? { link: transformLink(item.link, currentLang) } : {}),
  ...
  const normalizeNav = (nav: DefaultThemeConfig['nav'] | undefined, currentLang: string) => {
    if (!nav) {
      return []
    }
    const transformNavItem = <T extends NavItem>(navItem: T): T => {
      return {
        ...navItem,
        ...(navItem.text ? { text: textReplace(navItem.text, currentLang) } : {}),
        ...('link' in navItem ? { link: transformLink(navItem.link, currentLang) } : {}),
        ...('items' in navItem
          ? {
