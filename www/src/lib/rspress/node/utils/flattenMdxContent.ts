...
const fileSystem = fs as unknown as enhancedResolve.CachedInputFileSystem['fileSystem']
  ...
  const resolveResult = await new Promise<string>((resolve, reject) => {
    resolver.resolve(
      ...
      (err, filePath) => {
        ...
        if (!filePath) {
          return reject(new Error(`Empty result when resolving ${importPath} from ${importer}`))
        }
...
export async function flattenMdxContent(
  ...
  for (const importNode of importNodes) {
    ...
    try {
      absoluteImportPath = await resolveDepPath(importPath, path.dirname(basePath), alias)
    } catch (_e) {
      continue
    }
    ...
    if (MDX_OR_MD_REGEXP.test(absoluteImportPath)) {
      ...
      const { flattenContent: replacedValue, deps: subDeps } = await flattenMdxContent(
        importedContent,
        absoluteImportPath,
        alias,
      )
      ...
      result = result
        .replace(new RegExp(`import\\s+${id}\\s+from\\s+['"](${importPath})['"];?`), '')
