...
export class RoutePage {
  ...
  static createFromExternal(routePath: string, filepath: string, docDir: string): RoutePage {
  ...
  static generateRouteMeta(routePath: string, filepath: string, docDir: string): RouteMeta {
...
export function absolutePathToRelativePath(absolutePath: string, docsDir: string): string {
...
function absolutePathToRoutePrefix(absolutePath: string, docsDir: string): string {
  const relativePath = slash(
    path.relative(docsDir, absolutePath.replace(path.extname(absolutePath), '')),
  )
  const routeService = RouteService.getInstance()
...
export function addRoutePrefix(workDir: string, docsDir: string, link: string): string {
