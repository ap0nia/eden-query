...
<script lang="ts">
  ...
  const mutation = eden.posts({ id: "" }).post.createMutation({
    onSuccess(input) {
      utils.posts.invalidate()
      utils.posts({ id: input.id }).invalidate() // Will not invalidate queries for other id's 👍
    },
  })
</script>
```
...
<script lang="ts">
  ...
  const invalidatePostById = () => {
    ...
    utils.posts({ id: 1 }).get.invalidate()
  }

  // Example queries
  eden.index.get.createQuery() // Would only be validated by 1️⃣ only.
  eden.posts.get.createQuery() // Would be invalidated by 1️⃣ & 2️⃣
  eden.posts({ id: 1 }).get.createQuery() // Would be invalidated by 1️⃣, 2️⃣ and 3️⃣
  eden.posts({ id: 2 }).get.createQuery() // would be invalidated by 1️⃣ and 2️⃣ but NOT 3️⃣!
</script>
```
