...
<PackageManagerTabs command="add superjson" />

#### 2. Add the `transformPlugin` to your Elysia server application

```typescript twoslash include eq-transformers-application title=server.ts
...
<PackageManagerTabs command="add superjson devalue" />

#### 2. Create the transformer

```typescript twoslash include eq-transformers-custom-transformer title=transformer.ts
