...
Eden-Query is a type-safe client for [Elysia.js](https://elysiajs.com) that leverages
the asynchronous state management capabilities of [Tanstack Query](https://tanstack.com/query/latest).

<Integration />

The Eden-Query project is a collection of libraries that includes [`@ap0nia/eden`](/eden), the actual RPC client,
as well as framework-specific integrations, such as `@ap0nia/eden-react-query` and `@ap0nia/eden-svelte-query`
for React and Svelte respectively.
...
> [!NOTE]
> The [Eden-Query documentation](/eden-query) is an active work in progress
> and may contain links that point to the [tRPC documentation](https://trpc.io/docs)
> in order to supplement (possibly missing) information.
