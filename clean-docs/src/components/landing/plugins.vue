...
import { plugins as p } from '../../plugins'
const plugins = [
  ...
  ['Clerk', '/assets/clerk.webp', 'https://github.com/wobsoriano/elysia-clerk'],
  ['Lucia Auth', '/assets/lucia.svg', 'https://github.com/pilcrowOnPaper/lucia'],
  [
    'Logger',
    '/assets/hexagon.svg',
  ...
  >
    <section class="flex flex-col w-full max-w-lg">
      ...
      <p class="text-xl text-gray-400 w-full max-w-lg mb-4">
        Being one of the most popular choices for a Bun web framework, likely there is a plugin for
        what you want.
      </p>

      <p class="text-xl text-gray-400 w-full max-w-lg">
        If the plugin you need is not there, it's easy to create one and share it with the
        community.
      </p>
    ...
    <section class="flex flex-col w-full max-w-xl">
      ...
      >
        ...
        >
          <a
            :href="plugin.href"
            :target="plugin.href.startsWith('http') ? '_blank' : ''"
          ...
          >
            <img
              :src="isDark && plugin.darkIcon ? (plugin.darkIcon ?? plugin.icon) : plugin.icon"
              :alt="plugin.name"
              loading="lazy"
