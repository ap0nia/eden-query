  ...
  <section class="flex flex-col w-full max-w-6xl mx-auto mt-16">
    ...
    <ul id="sponsors" v-if="sponsors.length > 0" class="my-8">
      <li v-for="sponsor in sponsors" :key="sponsor.sponsorEntity.login">
        ...
        >
          <div class="w-16 h-16 rounded-full bg-gray-100 overflow-hidden mask mask-hexagon mb-3">
            <img
          ...
          <p className="text-sm ">
            {{ sponsor.sponsorEntity.name ?? sponsor.sponsorEntity.login ?? '[Private]' }}
          </p>
          <!-- <p
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 my-0">
            {{ dayjs().from(dayjs(sponsor.createdAt)).replace('in', 'for') }}
          </p>
        </a>
      </li>
      <li>
        ...
        >
          ...
          <p className="text-lg font-semibold text-pink-400 mb-2">And you</p>
